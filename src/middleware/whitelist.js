    ...
    return function (req, res, next) {
        ...
        if (!isIPInWhitelist(whitelist, clientIp)
            ...
            if (!noLogPaths.includes(req.path)) {
                console.warn(
                    color.red(
                        `Blocked connection from ${ipDetails}; User Agent: ${userAgent}\n\tTo allow this connection, add its IP address to the whitelist or disable whitelist mode by editing config.yaml in the root directory of your RoutTavern installation.\n`,
                    ),
                );
            }
